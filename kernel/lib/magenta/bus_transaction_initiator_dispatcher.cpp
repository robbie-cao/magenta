...
status_t BusTransactionInitiatorDispatcher::Pin(mxtl::RefPtr<VmObject> vmo, uint64_t offset,
    ...
    if (mapped_extents_len < size / PAGE_SIZE) {
    ...
    // Copy out addrs
    DEBUG_ASSERT(pmo->mapped_extents_len() <= size / PAGE_SIZE);
    *actual_mapped_extents_len = pmo->mapped_extents_len();
    const auto& pmo_addrs = pmo->mapped_extents();
    for (size_t i = 0; i < *actual_mapped_extents_len; ++i) {

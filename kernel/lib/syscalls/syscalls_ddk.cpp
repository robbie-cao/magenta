...
mx_status_t sys_bti_pin(mx_handle_t bti, mx_handle_t vmo, uint64_t offset, uint64_t size,
    ...
    if ((status = addrs.copy_array_to_user(mapped_addrs.get(), actual_len)) != MX_OK) {
        return status;
    }
    if ((status = actual_addrs_len.copy_to_user(static_cast<uint32_t>(actual_len))) != MX_OK) {
        return status;
    }
...
mx_status_t sys_bti_unpin(mx_handle_t bti, user_ptr<const uint64_t> addrs, uint32_t addrs_len) {
    ...
    static_assert(sizeof(dev_vaddr_t) == sizeof(uint64_t), "mismatched types");
    if ((status = addrs.copy_array_from_user(mapped_addrs.get(), addrs_len)) != MX_OK) {
        return status;
    }
